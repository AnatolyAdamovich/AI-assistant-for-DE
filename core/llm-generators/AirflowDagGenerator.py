...
from core.specs import AnalyticsSpec
...
class AirflowDagGenerator:
    def __init__(self, analytics_specification: AnalyticsSpec):
        ...
        self.llm_for_moving = ChatOpenAI(
                                model=settings.llm_airflow_moving_data,
        ...
        self.llm_for_args = ChatOpenAI(
                                model=settings.llm_airflow_args,
    ...
    def _generate_dag_args(self):
        ...
        system_template = (
            ...
            "(for example: schedule_interval=\"@daily\", start_date=datetime(2024, 1, 1))."
        )
        user_template = (
            "Business process:\n"
            "- name: {name}\n"
    ...
    def _generate_moving_data_function(self):        
    ...
    def fill_and_save_template(self):
        ...
        with open(settings.dags_dir, "w", encoding="utf-8") as f:
            f.write(template)
